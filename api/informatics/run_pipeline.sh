...
do
    ...
    cutadapt -a TGGAATTCTCGGGTGCCAAGG -u 4 -u -4 -m 10 -o public/$p.trimmed.fastq.gz public/$file > public/$ref_name/$p.trim.txt
	#cutadapt
	((i++))
	progress=$(bc -l <<< "scale=2;$i*100/$num_steps")
    ...
    #samtools
    samtools view -bS public/$p.sam > public/$p.bam
	((i++))
	progress=$(bc -l <<< "scale=2;$i*100/$num_steps")
    ...
    #samtools
    samtools sort public/$p.bam -o public/$p.sorted.bam
	((i++))
	progress=$(bc -l <<< "scale=2;$i*100/$num_steps")
    ...
    #samtools
    samtools index public/$p.sorted.bam
	((i++))
	progress=$(bc -l <<< "scale=2;$i*100/$num_steps")
    ...
    #samtools
    samtools idxstats public/$p.sorted.bam > public/$ref_name/$p.counts.txt
	((i++))
	progress=$(bc -l <<< "scale=2;$i*100/$num_steps")
